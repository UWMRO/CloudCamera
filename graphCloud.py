...
class CloudGraph(object):
    ...
    def run_analysis(self, name, expose, gain):
        ...
        print("Median = "+str(median)+", Mean = " +
              str(mean)+", Standard Dev = "+str(std))
    ...
    def plot_histogram(self, img, values, bins, img_out, stat_arr):
        ...
        ax[0, 0].text(-30, 1020, 'Exposure = '+str(exp)+' [s]',
                      size=16, color="white", horizontalalignment='left', )
        ax[0, 0].text(-30, 1060, 'Gain = '+str(stat_arr[4]),
                      size=16, color="white", horizontalalignment="left")
        ax[0, 0].text(1240, 980, 'Median = %.1f' % (stat_arr[0]),
                      size=16, color="white", horizontalalignment='right')
        ax[0, 0].text(1240, 1020, "Mean = %.2f" % (stat_arr[1]),
                      size=16, color="white", horizontalalignment='right')
        ax[0, 0].text(1240, 1060, 'Standard Dev = %.2f' % (
            stat_arr[2]), size=16, color="white", horizontalalignment='right')
        ...
        if self.heatStatus == 1:
            ...
            ax[0, 0].text(1000, 100, "Heat = Unknown (%s)" %
                          self.heatStatus, size=18, color="yellow")
        ...
        fig.savefig("latest.png", cmap="grey", transparent=True,
    ...
    def mapImg(self, imArr=None, name=None, map=None):
        ...
        plt.draw()
        plt.savefig(name, transparent=True, facecolor="black",
