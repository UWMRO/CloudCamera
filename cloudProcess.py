...
def indProc():
    arr = []
    p = subprocess.Popen(['ps -aux | grep CloudCam.py'], shell=True,
    ...
    p = subprocess.Popen(['ps -aux | grep wiggleCloud.py'], shell=True,
                         stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    q = p.stdout.readlines()
    run_gif = False
    for x, f in enumerate(q):
    ...
    p = subprocess.Popen(['ps -aux | grep clouduino_interface.py'], shell=True,
                         stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    q = p.stdout.readlines()
    run_rain = False
    for x, f in enumerate(q):
