...
# where it looks in the path for modules
...
class transfer(object):
    ...
    def openConnection(self, server=None, user=None):
        ...
        key = paramiko.RSAKey.from_private_key_file(
            keyfile, password='mro2015')
        self.ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        if proxy:
            self.ssh.connect(
                serv['hostname'], username=serv['user'], pkey=key, timeout=30, sock=proxy)
        else:
    ...
    def transProgress(self, trans, toBeTrans):
        sys.stdout.write("\rtransferred: {0:.0f} %".format(
            (trans / toBeTrans) * 100))
        sys.stdout.flush()
    ...
    def uploadFile(self, server=None, user=None, f_in=None, server_dir=None):
        ...
        self.ftp.put(f_in, os.path.join(server_dir, f_in),
                     callback=self.transProgress)
...
if __name__ == "__main__":
    ...
    # t.closeConnection()
    t.uploadFile('galileo.apo.nmsu.edu', 'jwhueh',
                 'test.png', 'public_html/CloudCamera/')
