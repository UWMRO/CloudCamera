...
class CloudGif(object):
    ...
    def hourWiggle(self, dir=None):
        ...
        if os.path.isfile(gifPath):
            os.remove(gifPath)
        fps = 5
        command = "nice -20 ffmpeg -y -f image2 -r " + \
            str(fps) + " -pattern_type glob -i 'gif/*.png' " + str(gifName)
    ...
    def dayWiggle(self):
        ...
        command = "ffmpeg -y -f image2 -r 6 -pattern_type glob -i '%s/*.png' %s" % (
            str(gifPath), str(dayName+'.mp4'))
